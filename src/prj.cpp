...
bool Prj::SaveCrops(Page& page) {
    ...
    for (wxRect area : GetSelectArea(page)) {
        ...
        int sample_per_pixel = image.GetOptionInt(wxIMAGE_OPTION_TIFF_SAMPLESPERPIXEL);
        int bits_per_sample = image.GetOptionInt(wxIMAGE_OPTION_TIFF_BITSPERSAMPLE);
        ...
        fs::path file_path =
            config.output_dir / std::format("{}-{}{}", page.image_path.stem().string(), count,
...
/*自动选择黑色像素区域
    ...
    expand_size: 留边空白大小
*/
static std::optional<wxRect> CalcuateSelectArea(cv::Mat image, int filter_noise_size,
...
const std::vector<wxRect>& Prj::GetSelectArea(Page& page) {
    if (page.modified) {
        ...
        cv::Mat image(ii.GetHeight(), ii.GetWidth(), CV_8UC3, static_cast<void*>(ii.GetData()));
        int prev_line = 0;
        for (int line : page.crop_lines) {
            cv::Mat sub_image = image.rowRange(prev_line, line);
            auto area = CalcuateSelectArea(sub_image, config.filter_noise_size, 0, prev_line);
...
std::optional<std::set<int>::iterator> Page::SearchNearestLine(int key, int limit) const {
...
bool Prj::InsertLineRecord::Do([[maybe_unused]] Prj& prj) {
    auto [_, actual_modified] = page.get().crop_lines.insert(line);
    page.get().modified = true;
    return actual_modified;
}

void Prj::InsertLineRecord::Undo([[maybe_unused]] Prj& prj) {
    page.get().crop_lines.erase(line);
    page.get().modified = true;
}

bool Prj::EraseLineRecord::Do([[maybe_unused]] Prj& prj) {
    page.get().crop_lines.erase(it);
    page.get().modified = true;
    return true;
}

void Prj::EraseLineRecord::Undo([[maybe_unused]] Prj& prj) {
...
static std::strong_ordering NaturalCompare(std::string_view a, std::string_view b) {
...
static std::vector<std::reference_wrapper<Page>> SortPages(std::vector<Page>& pages) {
    ...
    std::ranges::sort(
        pages_sorted,
        [](std::string_view s1, std::string_view s2) { return NaturalCompare(s1, s2) < 0; },
    ...
    auto extension_filter = [](const fs::directory_entry& entry) {
        return entry.is_regular_file() &&
               std::find(std::begin(VALID_EXTENSION), std::end(VALID_EXTENSION),
                         entry.path().extension().string()) != std::end(VALID_EXTENSION);
    };
